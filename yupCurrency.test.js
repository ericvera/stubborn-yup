...
describe('yupCurrency', () => {
  ...
  test('works and rounds many decimal values', async () => {
    const currency = yupCurrency('price', { required: true })
    const result = await currency.validate('1999923.4322')

    expect(result).toBe('1999923.43')
  })

  test('works and rounds numbers smaller than 0.005 to 0.00', async () => {
    const currency = yupCurrency('price', { required: true })
    const result = await currency.validate('0.004')
